...
public class GeneratorAgent : MonoBehaviour
{
    ...
    private void activateAgents()
    {
        if (agentOrder == 0)
        {
            ...
            {
                agent = Instantiate(mountainAgent, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0), transform).GetComponent<Agent>();
                agent.active = true;
            }
        }
        if (agentOrder == 1)
        {
            ...
            {
                agent = Instantiate(riverAgent, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0), transform).GetComponent<Agent>();
                agent.active = true;
            }
        }
        if (agentOrder == 2)
        {
            ...
            {
                agent = Instantiate(smoothingAgent, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0), transform).GetComponent<Agent>();
